...
# ESCOLHENDO ATIVOS PARA ENTRAR NA CARTEIRA
    ...
    if (sum(peso) != 1) & (n_stocks > 0):
        ...
        if st.sidebar.button('Gerar Carteira'):
            ...
            st.write(f'A variância do portfólio é {round(pfolio_var,3)}. A variância do Ibovespa é {float(round(benchmark_var,3))}.')
            st.write(f'A volatilidade do portfólio é {round(pfolio_vol,3)}. A volatilidade do Ibovespa é {float(round(benchmark_vol,3))}.')
